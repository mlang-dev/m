...
var_node *_create_var_node(const vector<pair<string, exp_node *>> &var_names,
...
prototype_node *create_prototype_node(const std::string &name,
...
condition_node *_create_condition_node(exp_node *condition, exp_node *then_node,
                                       exp_node *else_node) {
...
for_node *_create_for_node(const std::string &var_name, exp_node *start,
...
block_node *_create_block_node(exp_node *ret) {
  block_node *block = new block_node();
  block->base.type = NodeType::BLOCK_NODE;
  block->nodes.push_back(ret);
  return block;
}
...
exp_node *_parse_function_app_or_def(parser *parser, string id_name) {
  ...
  if (parser->curr_token_num != ';') parse_next_token(parser);
  if (parser->curr_token_num == '=') func_definition = true;
  ...
  if (func_definition) {
    ...
    prototype_node *prototype =
        create_prototype_node(id_name, argNames, false, 0);
...
exp_node *parse_statement(parser *parser, exp_node *parent) {
  ...
  if (parser->curr_token.type == TOKEN_IMPORT)
    ...
    node = _parse_exp(parser);
  else {
    string id_name = *parser->curr_token.ident_str;
    parse_next_token(parser);  // skip identifier
    if (parser->curr_token_num == '=') {
      // variable definition
      node = _parse_variable(parser, id_name);
    } else if (parser->curr_token_num == ';' ||
               g_op_precedences[parser->curr_token_num]) {
      // just id expression evaluation
      node = _parse_exp(parser, (exp_node *)_create_ident_node(id_name));
    } else {  // if(parser->curr_token_num=='('|| parser->curr_token.type ==
              // TOKEN_IDENT,)
      // function definition or application
      node = _parse_function_app_or_def(parser, id_name);
    }
  }
  node->parent = parent;
  return node;
...
exp_node *_parse_exp(parser *parser, exp_node *lhs) {
  if (!lhs) lhs = _parse_unary(parser);
  if (!lhs) return 0;
...
exp_node *_parse_prototype(parser *parser) {
  ...
  return (exp_node *)create_prototype_node(fun_name, arg_names, proto_type != 0,
                                           bin_prec);
}
...
exp_node *_parse_function_with_prototype(parser *parser,
  ...
  if (auto exp = _parse_exp(parser)) {
    return (exp_node *)_create_function_node((prototype_node *)prototype,
                                             _create_block_node(exp));
  }
  return 0;
}
...
exp_node *parse_function(parser *parser, bool has_fun_def_keyword) {
  if (has_fun_def_keyword) parse_next_token(parser);
...
exp_node *parse_exp_to_function(parser *parser, exp_node *exp, const char *fn) {
  if (!exp) exp = _parse_exp(parser);
  if (exp) {
    auto args = std::vector<std::string>();
    auto prototype = create_prototype_node(fn, args);
    auto block = _create_block_node(exp);
...
block_node *parse_block(parser *parser, exp_node *parent, void (*fun)(exp_node*, void*), void* jit) {
  int indent = !parent? 0:parent->indent;
  auto block = new block_node();
  block->base.type = BLOCK_NODE;
  block->base.parent = parent;
  block->base.indent = indent;
  while (true) {
    parse_next_token(parser);
    if (parser->curr_token.indent < indent || parser->curr_token.type == TOKEN_EOF) {
      break;
    }
