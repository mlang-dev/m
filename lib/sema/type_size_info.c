...
void _layout_field(struct type_context *tc, struct struct_layout *sl, struct type_item *field_type)
{
    struct type_size_info tsi = get_type_size_info(tc, field_type);
...
struct struct_layout *layout_struct(struct type_context *tc, struct type_item *to, enum ADTKind kind)
{
    ...
    for (u32 i = 0; i < member_count; i++) {
        ...
        _layout_field(tc, sl, field_type);
    }
    _layout_end(sl);
    return sl;
}
...
struct type_size_info _create_struct_type_size_info(struct type_context *tc, struct type_item *to, enum ADTKind kind)
{
    struct type_size_info ti;
    struct struct_layout *sl = layout_struct(tc, to, kind);
...
struct type_size_info _create_array_type_size_info(struct type_context *tc, struct type_item *at)
{
    struct type_size_info ti;
    struct type_size_info elm_ti = get_type_size_info(tc, at->val_type);
...
{
    if (type->name && hashtable_in_p(&tc->ts_infos, type->name)) {
        return *(struct type_size_info *)hashtable_get_p(&tc->ts_infos, type->name);
    }
    struct type_size_info ti;
    if (type->type == TYPE_ARRAY) {
        ti = _create_array_type_size_info(tc, type);
    } else if (is_struct_like_type(type)) {
        ti = _create_struct_type_size_info(tc, type, Product);
    } else if (type->type == TYPE_VARIANT) {
        ti = _create_struct_type_size_info(tc, type, Sum);
    } else {
        ti = _create_scalar_type_size_info(type);
    }
    if(type->name){
        hashtable_set_p(&tc->ts_infos, type->name, &ti);
    }
    return ti;
}
...
{
    struct type_size_info tsi = get_type_size_info(tc, type);
