...
void wasm_emit_copy_struct_value(struct type_context *tc, WasmModule ba, u32 to_var_index, u32 to_offset, struct type_item *type, u32 from_var_index, u32 from_offset)
{
    ...
    for (u32 i = 0; i < array_size(&type->args); i++) {
        ...
        u32 align = get_type_align(tc, field_type);
        if(field_type->type == TYPE_STRUCT){
            wasm_emit_copy_struct_value(tc, ba, to_var_index, to_offset + field_offset, field_type, from_var_index, from_offset + field_offset);
        }else{
