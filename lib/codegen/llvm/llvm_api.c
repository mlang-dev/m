...
struct address create_temp_mem(struct type_item *to, unsigned align, symbol name)
{
    (void)to;
    (void)align;
    (void)name;
...
LLVMValueRef _access_bytes_struct(struct cg_llvm *cg, LLVMBuilderRef builder, LLVMValueRef value, LLVMTypeRef type, uint64_t size)
{
    ...
    uint64_t struct_type_size = LLVMABISizeOfType(dl, type);
    uint64_t first_type_size = LLVMABISizeOfType(dl, first_type);
    ...
    if (LLVMGetTypeKind(first_type) == LLVMStructTypeKind) {
        return _access_bytes_struct(cg, builder, value, first_type, size);
    }
    return value;
}
...
void create_coerced_store(struct cg_llvm *cg, LLVMBuilderRef builder, LLVMValueRef src, LLVMValueRef dst, unsigned align)
{
    ...
    if (dst_kind == LLVMStructTypeKind) {
        dst = _access_bytes_struct(cg, builder, dst, dst_type, src_size);
...
LLVMValueRef create_coerced_load(struct cg_llvm *cg, LLVMBuilderRef builder, LLVMValueRef src, LLVMTypeRef dst_type, unsigned align)
{
    ...
    if (src_type_kind == LLVMStructTypeKind) {
        src = _access_bytes_struct(cg, builder, src, src_type, dst_size);
