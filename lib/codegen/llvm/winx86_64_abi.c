...
struct abi_arg_info _winx86_64_classify(struct target_info *ti, struct type_exp *type, unsigned *free_sse_regs, bool is_return_type, bool is_vector_call, bool is_reg_call)
{
    (void)free_sse_regs;
    (void)is_vector_call;
    (void)is_reg_call;
...
void winx86_64_compute_fun_info(struct target_info *ti, struct fun_info *fi)
{
    ...
    fi->ret = _winx86_64_classify(ti, fi->ret.type, &free_sse_regs, true, false, false);
    ...
    for (unsigned arg_no = 0; arg_no < array_size(&fi->args); arg_no++) {
        ...
        *aai = _winx86_64_classify(ti, aai->type, maybe_free_sse_regs, false, false, false);
    }
}
