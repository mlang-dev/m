...
struct fun_info *get_fun_info(struct ast_node *func_type)
{
    ...
    struct fun_info *result = hashtable_get_p(fun_infos, func_type->func_type->name);
    if (result)
        return result;
    struct fun_info fi;
    ...
    if (func_type->func_type->is_variadic)
        param_num -= 1;
    fun_info_init(&fi, func_type->func_type->is_variadic ? param_num : ALL_REQUIRED);
    ...
    hashtable_set_p(fun_infos, func_type->func_type->name, &fi);
    return (struct fun_info *)hashtable_get_p(fun_infos, func_type->func_type->name);
}
