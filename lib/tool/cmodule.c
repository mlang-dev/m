...
struct ast_node *create_function_func_type(CXCursor cursor)
{
    ...
    struct type_expr *ret_type = create_nullary_type(type, get_type_symbol(type));
    ...
    for (int i = 0; i < num_args; ++i) {
        ...
        struct ast_node *fun_param = var_node_new(var_name, annotated_type_name, false, 0, true, param_loc);
