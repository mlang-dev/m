...
TEST(test_parser, func_type_no_param)
{
    ...
    ASSERT_STREQ("int", string_get(node->ft->is_of_ret_type_node->ident->name));
    ast_node_free(block);
    parser_free(parser);
    frontend_deinit(fe);
}
...
TEST(test_parser, func_type_no_param_no_return)
{
    ...
    ASSERT_EQ(FUNC_TYPE_NODE, node->node_type);
    ASSERT_STREQ("print", string_get(node->ft->name));
    ASSERT_EQ(UNIT_NODE, node->ft->is_of_ret_type_node->node_type);
    ast_node_free(block);
    parser_free(parser);
    frontend_deinit(fe);
}
...
TEST(test_parser, ref_type)
{
    ...
    ASSERT_EQ(1, array_size(&block->block->nodes));
    ASSERT_EQ(VAR_NODE, node->node_type);
    ASSERT_STREQ("int", string_get(node->var->is_of_type->unop->operand->ident->name));
    ast_node_free(block);
    parser_free(parser);
    frontend_deinit(fe);
}
